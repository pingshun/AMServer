...
var log 		= require('log4js').getLogger("user_model");
...
module.exports = {
	...
	getByName: function (user_name, cb) {
		pool.getConnection(function(err, connection) {
            if (err) {
                console.error(err);
                req.session.error = 'error';
                ...
                connection.query($sql.queryByName, [user_name], function (err, result) {
                    if (cb) {
                        cb(err, result);
                    }
                    connection.release();
	...
	create: function (user_name, password, email, cb) {
		pool.getConnection(function(err, connection) {
			var user = {
				name: user_name,
				email: email,
			};
			var token = jwt.sign(user, 'AM_SECRET');

			connection.query($sql.insert, [user_name, password, email, token], function(err, result) {
				log.debug(result);
